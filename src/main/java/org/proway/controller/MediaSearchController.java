        ...
        for (Series sr :  netflixSystem.getSeriesCatalog().stream().toList())
        {
            ...
            if (sr.equals(this.filterMatchMedia(sr, seriesFilter)))
            {
                ...
                if (seriesFilter.getTotalEpisodesIntervalStart() != null && seriesFilter.getTotalEpisodesIntervalEnd() != null) {
                    itMatchesSomething = false;
                    if (sr.getTotalEpisodes() >= seriesFilter.getTotalEpisodesIntervalStart() && sr.getTotalEpisodes() <= seriesFilter.getTotalEpisodesIntervalEnd()){
                        itMatchesSomething = true;
                    }
                    if (!itMatchesSomething) {
                        continue;
                ...
                if (seriesFilter.getTotalEpisodesIntervalStart() == null && seriesFilter.getTotalEpisodesIntervalEnd() != null) {
                    itMatchesSomething = false;
                    if (sr.getTotalEpisodes() <= seriesFilter.getTotalEpisodesIntervalEnd()){
                        itMatchesSomething = true;
                    }
                    if (!itMatchesSomething) {
                        continue;
